...
void CLK_ALARM_Start(void) 
{
    ...
    CLK_ALARM_CLKEN |= CLK_ALARM_CLKEN_MASK;
	CLK_ALARM_CLKSTBY |= CLK_ALARM_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_ALARM_Stop
********************************************************************************
*
* Summary:
...
void CLK_ALARM_Stop(void) 
{
    ...
    CLK_ALARM_CLKEN &= (uint8)(~CLK_ALARM_CLKEN_MASK);
	CLK_ALARM_CLKSTBY &= (uint8)(~CLK_ALARM_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_ALARM_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_ALARM_CLKEN & CLK_ALARM_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_ALARM_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_ALARM_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_ALARM__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_ALARM_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_ALARM_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_ALARM_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_ALARM_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_ALARM_GetSourceRegister();
    uint16 oldDivider = CLK_ALARM_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_ALARM_CLKEN & CLK_ALARM_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_ALARM_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLK_ALARM_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_ALARM_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_ALARM_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_ALARM_MOD_SRC |= modeBitMask & (uint8)CLK_ALARM_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_ALARM_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_ALARM_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_ALARM_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_ALARM_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_ALARM_MOD_SRC & (uint8)(CLK_ALARM_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_ALARM_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_ALARM_GetDividerRegister();
    uint8 oldSrc = CLK_ALARM_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_ALARM_MOD_SRC |= CYCLK_SSS;
        CLK_ALARM_MOD_SRC =
            (CLK_ALARM_MOD_SRC & (uint8)(~CLK_ALARM_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_ALARM_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_ALARM_MOD_SRC & CLK_ALARM_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_ALARM_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_ALARM_PHASE = clkPhase & CLK_ALARM_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_ALARM_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_ALARM_PHASE & CLK_ALARM_PHASE_MASK;
}
