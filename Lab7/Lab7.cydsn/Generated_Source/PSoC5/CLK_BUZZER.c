...
/*******************************************************************************
* Function Name: CLK_BUZZER_Start
********************************************************************************
*
* Summary:
...
void CLK_BUZZER_Start(void) 
{
    ...
    CLK_BUZZER_CLKEN |= CLK_BUZZER_CLKEN_MASK;
	CLK_BUZZER_CLKSTBY |= CLK_BUZZER_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_BUZZER_Stop
********************************************************************************
*
* Summary:
...
void CLK_BUZZER_Stop(void) 
{
    ...
    CLK_BUZZER_CLKEN &= (uint8)(~CLK_BUZZER_CLKEN_MASK);
	CLK_BUZZER_CLKSTBY &= (uint8)(~CLK_BUZZER_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_BUZZER_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_BUZZER_CLKEN & CLK_BUZZER_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_BUZZER_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_BUZZER_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_BUZZER__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_BUZZER_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_BUZZER_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_BUZZER_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_BUZZER_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_BUZZER_GetSourceRegister();
    uint16 oldDivider = CLK_BUZZER_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_BUZZER_CLKEN & CLK_BUZZER_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_BUZZER_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLK_BUZZER_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_BUZZER_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_BUZZER_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_BUZZER_MOD_SRC |= modeBitMask & (uint8)CLK_BUZZER_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_BUZZER_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_BUZZER_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_BUZZER_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_BUZZER_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_BUZZER_MOD_SRC & (uint8)(CLK_BUZZER_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_BUZZER_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_BUZZER_GetDividerRegister();
    uint8 oldSrc = CLK_BUZZER_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_BUZZER_MOD_SRC |= CYCLK_SSS;
        CLK_BUZZER_MOD_SRC =
            (CLK_BUZZER_MOD_SRC & (uint8)(~CLK_BUZZER_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_BUZZER_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_BUZZER_MOD_SRC & CLK_BUZZER_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_BUZZER_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_BUZZER_PHASE = clkPhase & CLK_BUZZER_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_BUZZER_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_BUZZER_PHASE & CLK_BUZZER_PHASE_MASK;
}
